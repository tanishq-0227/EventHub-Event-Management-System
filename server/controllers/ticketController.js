...
const parseBool = (value) => value === true || value === 'true' || value === 'on';

const buildTicketPayload = (body, event = null) => ({
  ...body,

  price: Number(body.price) || 0,
  totalQuantity: Number(body.totalQuantity) || 1,
  perUserLimit: Number(body.perUserLimit) || 5,

  venueLayoutType: body.venueLayoutType || event?.venueLayoutType || 'standing',

  sectionName: body.sectionName || 'General',

  allowSeatSelection: parseBool(body.allowSeatSelection),

  seatRows: Number(body.seatRows) || event?.seatingConfig?.rows || 0,

  seatsPerRow: Number(body.seatsPerRow) || event?.seatingConfig?.seatsPerRow || 0,
});
...
const createTicket = async (req, res) => {
  ...
  const payload = buildTicketPayload(req.body, event);
...
const updateTicket = async (req, res) => {
  ...
  const payload = buildTicketPayload(req.body, ticket.event);

  const updated = await Ticket.findByIdAndUpdate(req.params.id, payload, {
    new: true,
    runValidators: true,
  });
...
const deleteTicket = async (req, res) => {
  ...
  if (ticket.soldQuantity > 0) {
    throw new ApiError(
      400,
      `Cannot delete a ticket that has ${ticket.soldQuantity} sold units`
    );
  }
  ...
  try {
    ...
    if (err.name === 'TokenExpiredError') {
      return res.status(400).json(
        new ApiResponse(
          400,
          { result: 'expired' },
          'Ticket QR has expired — event has ended'
        )
      );
    }
    ...
    return res.status(400).json(
      new ApiResponse(
        400,
        { result: 'invalid' },
        'Invalid QR code — token is tampered or unrecognised'
      )
    );
  }
  ...
  try {
    ...
    if (ticket) {
      if (ticket.booking?.status === 'cancelled') {
        await IssuedTicket.findByIdAndUpdate(
          ticket._id,
          {
            $set: {
              isUsed: false,
    ...
    if (existing.isUsed) {
      ...
      return res.status(409).json(
        new ApiResponse(
          ...
          `🚫 Already used — entry was granted at ${
            existing.usedAt?.toLocaleString('en-IN') ?? 'unknown time'
          }`
        )
      );
    }
...
const getFraudReport = async (req, res) => {
    ...
    IssuedTicket.aggregate([
      { $match: { event: new mongoose.Types.ObjectId(eventId) } },
      {
        $group: {
          ...
          fraudulent: {
            $sum: { $cond: [{ $gt: ['$scanAttempts', 1] }, 1, 0] },
          },
  ...
  res.json(
    new ApiResponse(
      200,
      { summary: summary[0] ?? {}, suspicious },
      'Fraud report fetched'
    )
  );
};
